...
ast * new_ast_program(arena * ar, ast * fn) {
    assert(ar);
    assert(fn);
...
ast * new_ast_function(arena * ar, char * name, ast * body) {
    assert(ar);
    assert(name);
    assert(body);
    ast * a    = arena_alloc_type(ar, ast);
...
ast * new_ast_error_exp_token(arena *    ar,
...
ast * new_ast_error_exp_ast(arena * ar, ast * next, ast_type exp_ast_type) {
    assert(ar);
    ast * a             = arena_alloc_type(ar, ast);
    a->type             = AST_ERR;
    a->err.next         = next;
...
ast * new_ast_error_expr(arena * ar) {
    assert(ar);
    ast * a     = arena_alloc_type(ar, ast);
    a->type     = AST_ERR;
    a->err.next = NULL;
    a->err.type = AST_ERR_EXPR;
    return a;
}

ast * new_stmt_return(arena * ar, ast * expr) {
    assert(ar);
    assert(expr);
...
ast * new_expr_constant(arena * ar, int value) {
    assert(ar);
    ast * a       = arena_alloc_type(ar, ast);
    a->type       = AST_EXPR;
...
void print_ast(FILE * stream, ast * a, int indent) {
    assert(stream);
    assert(a);
