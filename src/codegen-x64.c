...
void
asm_instr_codegen_x64(FILE *out,
                      asm_instr instr)
{
    switch (instr.type) {
    case INSTR_MOV:
...
void
asm_function_codegen_x64(FILE *out,
                         asm_function *fn)
{
    assert(fn);
    assert(fn->name);
...
void
asm_program_codegen_x64(FILE *out,
                        asm_program *p)
{
    assert(p);
