str_buffer make_str_buffer(arena * ar, u64 size) {
...
bool str_fits_in_buffer(str_buffer * strb, u64 len) {
    return len + 1 + strb->pos <= strb->size;
}

char * append_str_to_buffer(str_buffer * strb, char * str, u64 len) {
...
str_store make_str_store(arena * ar, u64 buffer_size, u64 num_buckets) {
    str_store st   = {0};
...
char * intern_str_n(str_store * st, char * str, u64 len) {
...
char * intern_str(str_store * st, char * str) {
    u64 len = strlen(str);
    return intern_str_n(st, str, len);
}

char * find_stored_str_n(str_store * st, char * str, u64 len) {
    u64        bucket_index = hash_str(str, len) % st->num_buckets;
    str_node * bucket       = st->table[bucket_index];
    for (str_node * node = bucket; node; node = node->next) {
        assert(node->str);
...
char * find_stored_str(str_store * st, char * str) {
    u64 len = strlen(str);
...
void test_str_store(void) {
    ...
    assert(s2);
    assert(str_eq(s2, "orange"));
    assert(s1r);
    assert(str_eq(s1r, "apple"));
    assert(s1r == s1);
    assert(s3);
    assert(str_eq(s3, "grape"));
    assert(s4);
    assert(str_eq(s4, "avocado"));
    assert(s5);
    assert(str_eq(s5, "pear"));
    assert(s6);
    assert(str_eq(s6, "banana"));
    assert(s7);
    assert(str_eq(s7, "acai"));
    assert(s8);
    assert(str_eq(s8, "grapefruit"));
    assert(s5r);
    assert(str_eq(s5r, "pear"));
    assert(s5r == s5);
    assert(s9);
    assert(str_eq(s9, "lemon"));
    assert(sa);
    assert(str_eq(sa, "tamarind"));
    assert(sb);
    assert(str_eq(sb, "ata"));
    assert(s1rr);
    assert(str_eq(s1rr, "apple"));
    assert(s1rr == s1);
    assert(sar);
    assert(str_eq(sar, "tamarind"));
    assert(sar == sa);
